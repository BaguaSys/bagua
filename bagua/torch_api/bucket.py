...
class BaguaBucket:
    ...
    def append_asynchronous_model_average_op(self, peer_selection_mode: str):
        ...
        This operation is intended to run in parallel with the computation process. In some cases, for example
        gradient computation, you should keep the model constant. The Bagua backend uses a lock for exclusive access
        to the model. Call ``op.lock_weight()`` to acquire the lock and ``op.unlock_weight()`` to release it.
